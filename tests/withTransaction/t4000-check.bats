...
@test "command can check whether its transaction is still valid after the timeout" {
    ...
    [ $status -eq 0 ]
    [[ "$output" =~ ^"ERROR: Current transaction timed out "[12]" second"s?" ago.
Warning: Current transaction timed out "[12]" second"s?" ago."$ ]] || echo "$output" | failThis prefix \# >&3
...
@test "transaction check error can be suppressed when the current transaction timed out" {
    ...
    [ $status -eq 0 ]
    [[ "$output" =~ ^"Warning: Current transaction timed out "[12]" second"s?" ago."$ ]] || echo "$output" | failThis prefix \# >&3
    assert_file "6-invalid"
}
